...
import { Search, Plus, Send, Paperclip, MoreVertical, Check, CheckCheck } from 'lucide-react';
...
const THREAD = [
  { id: 1, from: 'Mrs. Patricia Lee', me: false, time: '10:30 AM', text: "Good morning! I wanted to share that Emma has shown remarkable improvement in her mathematics this term. Her assignment scores have consistently been above 90%." },
  { id: 2, from: 'me', me: true, time: '10:35 AM', text: "That's wonderful to hear! We've noticed her enthusiasm has really picked up as well. Is there anything we can do to further support her learning?" },
  { id: 3, from: 'Mrs. Patricia Lee', me: false, time: '10:38 AM', text: "She would benefit greatly from the advanced problem sets I have prepared. I can send them home for extra practice if you'd like." },
  { id: 4, from: 'Mrs. Patricia Lee', me: false, time: '10:42 AM', text: "Also, I wanted to flag that the mid-term exams are scheduled for April 7–11. Emma is well prepared but a bit of revision over the break would be helpful." },
];
  ...
  return (
    <AppLayout pageTitle="Messages">
      <div className="h-[calc(100vh-8rem)] flex gap-4 min-h-0">
        ...
        {/* Sidebar */}
        <Card className="w-[300px] shrink-0 border-slate-200 shadow-sm flex flex-col min-h-0">
        ...
        <Card className="flex-1 border-slate-200 shadow-sm flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {THREAD.map(m => (
              <div key={m.id} className={`flex gap-3 ${m.me ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[70%] ${m.me ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
          ...
          {/* Compose */}
          <div className="px-6 pb-5 pt-4 border-t border-slate-100 shrink-0">
            <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2">
              ...
              <button
                onClick={() => setMessage('')}
                className="bg-[#1e3a5f] hover:bg-[#152a45] text-white p-2 rounded-lg transition"
              >
