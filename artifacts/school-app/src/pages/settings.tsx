...
import { School, CalendarDays, Users, Bell, Palette, Save, Plus, Check } from 'lucide-react';
                ...
                {TABS.map(tab => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 md:gap-2.5 px-3 py-2 md:py-2.5 rounded-lg text-sm font-medium whitespace-nowrap shrink-0 md:shrink md:w-full text-left transition-colors ${
                        activeTab === tab.id
          ...
          {activeTab === 'school' && (
            <Card className="border-slate-200 shadow-sm">
              ...
              <CardContent className="p-4 md:p-6 space-y-5">
                <div className="flex items-center gap-4 md:gap-6 pb-5 border-b border-slate-100">
                  <div className="h-16 w-16 md:h-20 md:w-20 rounded-2xl bg-[#1e3a5f] flex items-center justify-center shrink-0">
                    <School className="h-8 w-8 md:h-10 md:w-10 text-sky-400" />
                  </div>
                  <div>
              ...
              <CardHeader className="border-b border-slate-100">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-0.5">Set terms, holidays, and grading periods</p>
                  </div>
                  <Button size="sm" className="gap-2 bg-[#1e3a5f] hover:bg-[#152a45] text-white h-8 shrink-0">
              ...
              <CardContent className="p-4 md:p-6 space-y-4">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={t.term} className="flex items-center justify-between bg-slate-50 rounded-xl px-4 py-3 border border-slate-100 gap-3 flex-wrap">
                      <div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2 text-center text-xs">
                    {[['A', '90–100'], ['B', '80–89'], ['C', '70–79'], ['D', '60–69'], ['F', '0–59']].map(([g, r]) => (
                      <div key={g} className="bg-slate-50 border border-slate-100 rounded-lg px-1 py-2">
                        <p className="font-bold text-slate-800">{g}</p>
                        <p className="text-slate-400 text-[10px] md:text-xs">{r}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <Card className="border-slate-200 shadow-sm">
              <CardHeader className="border-b border-slate-100">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-0.5">Manage access levels for each role</p>
                  </div>
                  <Button size="sm" className="gap-2 bg-[#1e3a5f] hover:bg-[#152a45] text-white h-8 shrink-0">
              ...
              <CardContent className="p-0">
                <div className="divide-y divide-slate-100">
                  {ROLES.map(r => (
                    <div key={r.name} className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 md:px-6 py-4 hover:bg-slate-50 transition-colors gap-2">
                      <div className="flex items-center gap-3">
                        <Badge className={`${r.color} border-0 shrink-0`}>{r.name}</Badge>
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <button className="text-xs text-sky-600 hover:underline font-medium shrink-0">Edit</button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {NOTIF_SETTINGS.map(cat => (
                  <div key={cat.category}>
                    ...
                    <div className="space-y-2">
                      {cat.items.map(item => (
                        <label key={item} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100 cursor-pointer hover:bg-slate-100 transition gap-3">
                          <span className="text-sm text-slate-700">{item}</span>
                          <button
                            onClick={() => setNotifs(prev => ({ ...prev, [item]: !prev[item] }))}
                            className={`relative h-5 w-9 rounded-full transition-colors shrink-0 ${notifs[item] ? 'bg-[#1e3a5f]' : 'bg-slate-300'}`}
                          >
            ...
            <Card className="border-slate-200 shadow-sm">
              ...
              <CardContent className="p-4 md:p-6 space-y-6">
                ...
                <div>
                  ...
                  <select className="h-10 px-3 border border-slate-200 rounded-lg text-sm text-slate-700 bg-white w-full max-w-xs">
